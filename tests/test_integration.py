...
class TestIntegration(unittest.TestCase):
    def test_module_interaction(self):
        ...
        # Test LQR controller with environment
    ...
    def test_controller_switching(self):
        ...
        # Linear controller
        lin = LinearController(K=jnp.array([10., 0., 5., 0.])).jit()
        # LQR controller  
        lqr = LQRController.from_linearisation(params).jit()
        
        init_state = jnp.array([0.05, 0.05, 0.0, 0.0])
        
        # Both should be able to handle the same initial condition
        sol_lin = simulate(lin, params, (0., 0.2), tgrid, init_state)
        sol_lqr = simulate(lqr, params, (0., 0.2), tgrid, init_state)
        
        assert sol_lin.ys.shape == sol_lqr.ys.shape
